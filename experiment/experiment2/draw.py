...
def plot_traj(args, init_info, uav_traj, expname, save, fair_service):
    ...
    for i, traj in enumerate(uav_traj):
        ...
        uav0_trajectory, = ax.plot([uav_pos[0][0], uav_new_pos[0][0]], [uav_pos[0][1], uav_new_pos[0][1]],
                                   linestyle='--', color=color[0], linewidth=2)
        uav1_trajectory, = ax.plot([uav_pos[1][0], uav_new_pos[1][0]], [uav_pos[1][1], uav_new_pos[1][1]],
                                   linestyle='--', color=color[1], linewidth=2)
        uav2_trajectory, = ax.plot([uav_pos[2][0], uav_new_pos[2][0]], [uav_pos[2][1], uav_new_pos[2][1]],
                                   linestyle='--', color=color[2], linewidth=2)
        uav3_trajectory, = ax.plot([uav_pos[3][0], uav_new_pos[3][0]], [uav_pos[3][1], uav_new_pos[3][1]],
                                   linestyle='--', color=color[3], linewidth=2)
        uav_pos = uav_new_pos
        if i == 28:
            plt.annotate(
                "t = 28",
                xy=(uav_pos[3][0], uav_pos[3][1]),
                xytext=(280, 210),
                arrowprops=dict(
                    facecolor='#3d3d3d',
                    shrink=0.05,
    ...
    ax.legend(handles=[uav_init, eve_init, gts_init,
                                          ...
                                          uav2_trajectory, uav3_trajectory,
                                          uav_final],
    ...
    # uav0_final_pos = ax.scatter(uav_pos[0][0], uav_pos[0][1], color="#0652dd", marker='*', s=120)
    # uav1_final_pos = ax.scatter(uav_pos[1][0], uav_pos[1][1], color="#0652dd", marker='*', s=120)
    # uav2_final_pos = ax.scatter(uav_pos[2][0], uav_pos[2][1], color="#0652dd", marker='*', s=120)
    # uav3_final_pos = ax.scatter(uav_pos[3][0], uav_pos[3][1], color="#0652dd", marker='*', s=120)
    # ax.legend(handles=[uav_init, eve_init, gts_init,
    #                    uav0_trajectory, uav1_trajectory,
    #                    uav2_trajectory, uav3_trajectory,
    #                    uav0_final_pos, uav1_final_pos,
    #                    uav2_final_pos, uav3_final_pos],
    #           labels=['Initial Positions', 'Eves', 'GTs',
    #                   'UAV-1', 'UAV-2',
    #                   'UAV-3', 'UAV-4',
    #                   'Final Position of UAV-1', 'Final Position of UAV-2',
    #                   'Final Position of UAV-3', 'Final Position of UAV-4'],
    #           loc="upper center",
...
def plot_jamming_powers(jamming_powers, expname, save, fair_service):
    ...
    if fair_service:
        plt.annotate(
            "t = 28",
            xy=(28, np.float32(jamming_powers[27][3])),
            xytext=(12, 0.033),
            arrowprops=dict(
                facecolor='black',
                shrink=0.05,
    ...
    if save:
        ...
        plt.savefig('./pics/{}_jamming_power.eps'.format(expname), dpi=600)
...
if __name__ == '__main__':
    ...
    path = '/home/zlj/uav_rsma-master/experiment/experiment2/'
    if fair_service:
        path = path + 'ltf_4uav'
    else:
        path = path + 'nltf_4uav'
