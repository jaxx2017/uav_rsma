...
def plot_trainReturns_cmp(train_returns_ltf, train_returns_nltf, save):
    ...
    plt.plot(indices, source_data_nltf, color='#ff4757', alpha=0.2)  # 透明度
    plt.plot(indices, smoothed_data_nltf, color='#ff4757', label='CQF')
    plt.plot(indices, source_data_ltf, color='#f0932b', alpha=0.2)  # 透明度
    plt.plot(indices, smoothed_data_ltf, color='#f0932b', label='FSF')
    plt.legend(loc='best')

    if save:
        plt.savefig('./pics/trainReturnsComp_2uav.png', dpi=600)
        plt.savefig('./pics/trainReturnsComp_2uav.eps', dpi=600)
...
def plot_testReturns_cmp(test_returns_ltf, test_returns_nltf, save):
    ...
    if save:
        plt.savefig('./pics/testReturnsComp_2uav.png', dpi=600)
        plt.savefig('./pics/testReturnsComp_2uav.eps', dpi=600)
...
def plot_fair_index_cmp(fair_index_ltf, fair_index_nltf, save):
    ...
    # plt.plot(indices, fair_index_ltf, label='FSF', color='#f0932b')
    # plt.plot(indices, fair_index_nltf, label='CQF', color='#ff4757')
    ...
    if save:
        plt.savefig('./pics/fair_idxComp_2uav.png', dpi=600)
        plt.savefig('./pics/fair_idxComp_2uav.eps', dpi=600)
...
def plot_ssr_cmp(ssr_ltf, ssr_nltf, save):
    ...
    if save:
        plt.savefig('./pics/ssrComp_2uav.png', dpi=600)
        plt.savefig('./pics/ssrComp_2uav.eps', dpi=600)
...
def plot_throughput_cmp(throughput_ltf, throughput_nltf, save, smooth=False):
    ...
    if save:
        plt.savefig('./pics/ThroughputComp_2uav.png', dpi=600)
        plt.savefig('./pics/ThroughputComp_2uav.eps', dpi=600)
...
def plot_reward_cmp(reward_ltf, reward_nltf, save):
    ...
    if save:
        plt.savefig('./pics/RewardComp_2uav.png', dpi=600)
        plt.savefig('./pics/RewardComp_2uav.eps', dpi=600)
...
def plot_throughput_gt(throughput_gt_nltf, throughput_gt_ltf, init_info, save):
    ...
    plt.ylim((0, 350))
    if save:
        plt.savefig('./pics/throughput_gt_2uav.png', dpi=600)
        plt.savefig('./pics/throughput_gt_2uav.eps', dpi=600)

    # plt.show()

if __name__ == '__main__':
    path_root = '/home/zlj/uav_rsma-master/experiment/experiment3/'   #TODO
    path_ltf = path_root + 'ltf_2uav'
    path_nltf = path_root + 'nltf_2uav'
